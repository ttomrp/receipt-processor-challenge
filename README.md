...
The application is written in Javascript using Node.js and Express web application framework. This branch contains a docker file per the instructions below. After cloning the repository run the following comands to set up the docker container and run the application.

```text
docker build -t receipt-processor .
docker run --publish 3000:3000 receipt-processor
```

The server will be running on localhost:3000. Included is a Postman collection for easier testing. Note that after successful completion of the `POST` request you will need to use the generated uuid in the response for the `GET` request.

Please reach out to me if you have any questions or issues at tomrperez@gmail.com.
